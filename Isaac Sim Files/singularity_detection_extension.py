...
class DetectionUI(BaseSampleUITemplate):
    ...
    def post_load_button_event(self):
        ...
        self.task_ui_elements["Generate Red Map"].enabled = True
        self.task_ui_elements["Generate Yellow Map"].enabled = True
        self.task_ui_elements["Generate Green Map"].enabled = True
    ...
    def _on_update(self, event):
        ...
        if getattr(self.sample, "ellipsoid", True):
            if getattr(self.sample, "velocity_ellipsoid") is not None:
                print(self.sample.ellipsoid)
    ...
    def on_show_ellipsoid(self):
        ...
        if self.sample.velocity_ellipsoid is None:
            ...
            eepos = ee_pos.reshape(-1, 3)[0]
            eequat = ee_quat.reshape(-1, 4)[0]
    ...
    def _on_generate_manip_map_button_color(self, color):
        self.sample._on_generate_manip_map_color(self.task_ui_elements["Input Directory"].get_value_as_string(), color)
        return
    ...
    def build_manip_map_ui(self):
        with ui.VStack(spacing=5):
            dict = {
                ...
                "type": "stringfield",
                "default_val": os.path.join(os.getcwd(), "Isaac_Sim_Full_File_Try3.json"),
            ...
            dict = {
                "label": "Generate Red Map",
                "type": "button",
                "text": "Generate Red Map",
                "tooltip": "Generate Red Map",
                "on_clicked_fn": lambda: self._on_generate_manip_map_button_color("Red"),
            }

            self.task_ui_elements["Generate Red Map"] = btn_builder(**dict)
            self.task_ui_elements["Generate Red Map"].enabled = False

            dict = {
                "label": "Generate Yellow Map",
                "type": "button",
                "text": "Generate Yellow Map",
                "tooltip": "Generate Yellow Map",
                "on_clicked_fn": lambda:self._on_generate_manip_map_button_color("Yellow"),
            }

            self.task_ui_elements["Generate Yellow Map"] = btn_builder(**dict)
            self.task_ui_elements["Generate Yellow Map"].enabled = False

            dict = {
                "label": "Generate Green Map",
                "type": "button",
                "text": "Generate Green Map",
                "tooltip": "Generate Green Map",
                "on_clicked_fn": lambda:self._on_generate_manip_map_button_color("Green"),
            }

            self.task_ui_elements["Generate Green Map"] = btn_builder(**dict)
            self.task_ui_elements["Generate Green Map"].enabled = False
